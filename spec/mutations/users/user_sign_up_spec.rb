    ...
    RSpec.describe UserSignUp, type: :request do
      ...
      describe 'Happy Paths' do
        it 'A user can be created' do
          user = TomoApiSchema.execute(@user_sign_up,
      ...
      describe 'Sad Paths' do
        ...
        it 'A user cannot be created with a passwordConfirmation that doesnt match password' do
          bob = TomoApiSchema.execute(@user_sign_up,
                                      variables: { input: { params: { email: 'JB@email.com', username: 'Jim Bobby', password: '1234',
                                                                      passwordConfirmation: 'DOESNTMATCH' } } })

          expect(bob['errors'][0]['message']).to eq('passwords must match')
        end

        it "A user cannot be created with an unknown attribute" do 
            bob = TomoApiSchema.execute(@user_sign_up, variables: { input: { params: { email: "JB@email.com", name: "Jim Bobby", password: "1234", passwordConfirmation: "1234" } }})

            expect(bob["errors"][0]["message"]). to eq("Variable $input of type UserSignUpInput! was provided invalid value for params.name (Field is not defined on UserInput)")
        end

        it 'A user cannot be created with a missing passwordConfirmation' do
          bob = TomoApiSchema.execute(@user_sign_up,
                                      variables: { input: { params: { email: 'JB@email.com',
                                                                      username: 'Jim Bobby', password: '1234' } } })
