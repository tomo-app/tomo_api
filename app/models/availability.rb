class Availability < ApplicationRecord
  ...
  def find_availabilities_to_pair
    ...
    people_to_pair = UserLanguage.where.not(user_id: user_id).where(fluency_level: 'target',
                                                                    language_id: native.language_id).where(user_id: target_users).order(:created_at)
